...
public class AppUserRestController {
    ...
    public ResponseEntity<AppUser> userById(@PathVariable Long id) {
        Object appUser = appUserRepository.findById(id);
        if (appUser == null) {
    ...
    public ResponseEntity<AppUser> deleteUser(@PathVariable Long id) {
        ...
        if (appUser == null) {
            return new ResponseEntity<AppUser>(HttpStatus.NO_CONTENT);
        } else if (((AppUser) appUser).getUsername().equalsIgnoreCase(loggedUsername)) {
            throw new RuntimeException("You cannot delete your account");
        } else {
            appUserRepository.delete((AppUser) appUser);
            return new ResponseEntity<AppUser>((AppUser) appUser, HttpStatus.OK);
        }
